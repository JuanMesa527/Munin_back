...
export class DeepSeekLlmAdapter implements LlmPort {
  ...
  async converseIntake(input: ConverseIntakeInput): Promise<Result<ConverseIntakeResult>> {
    const systemPrompt = [
      ...
      `La UNICA pregunta que la interfaz va a mostrar despues de tu respuesta es: "${input.preguntaAnclada}". Tu respuestaBot DEBE terminar preguntando exactamente eso — podes parafrasearla con tu tono, pero NUNCA combinarla con otro dato ni inventar una pregunta distinta.`,
