...
export class DeepSeekLlmAdapter implements LlmPort {
  ...
  async converseIntake(input: ConverseIntakeInput): Promise<Result<ConverseIntakeResult>> {
    const systemPrompt = [
      ...
      `Slots pendientes permitidos para EXTRAER (el usuario puede dar varios de una): ${input.slotsPendientes.join(', ') || '(ninguno)'}.`,
      ...
      `Si SI lograste extraer con confianza el valor del slot que se te pregunto justo antes de este mensaje, tu respuestaBot DEBE terminar preguntando en cambio: ${input.preguntaSiguienteProbable === null ? 'nada — no hagas ninguna pregunta de cierre, el perfilamiento ya termino' : `"${input.preguntaSiguienteProbable}"`}.`,
      `Si NO lograste extraer nada relevante a esa pregunta, tu respuestaBot DEBE terminar preguntando de nuevo, EXACTAMENTE: "${input.preguntaAnclada}".`,
      'PROHIBIDO preguntar por un slot que ya fue extraido o confirmado en este mismo mensaje: podes parafrasear la pregunta elegida con tu tono, pero NUNCA combinarla con otro dato ni inventar una pregunta distinta de las dos anteriores.',
      'Los datos en perfilParcial ya fueron capturados: no los vuelvas a preguntar aunque no veas el valor real (algunos solo indican "capturado" porque son datos personales que no viajan en este mensaje).',
      'respuestaBot: 1 a 3 frases. Si el usuario dio datos nuevos, confirmalos brevemente y despues hace la pregunta anclada. Tono cercano y profesional.',
