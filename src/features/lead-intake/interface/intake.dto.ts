/**
 * DTOs zod de F1 (lead-intake). Capa: interface (borde HTTP). Unico lugar donde
 * el payload crudo del cliente se valida antes de tocar `application/`. `texto`
 * se acota a 500 chars: higiene y superficie de prompt injection (regla 12/).
 */
...
/**
 * `/consent` NUNCA acepta `leadId` ("client-controlled identifier"): el schema
 * simplemente no lo declara. zod descarta cualquier clave desconocida por
 * defecto (ver comentario en `validate.ts`), asi que un `leadId` inyectado por
 * el cliente queda "stripped" antes de llegar al caso de uso, sin necesidad de
 * rechazar el request completo.
 */
